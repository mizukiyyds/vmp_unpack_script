...
;VMP3.x需要的变量
VAR nearest_call                         ;最近的call地址
VAR nearest_jcc                          ;最近的jcc地址
VAR nearest_ret                          ;最近的ret地址
;----------------------------脚本主逻辑-----------------------------------------
...
Loop_FixCall:
  ...
  Loop_FindLastRetn1:                            ;开始获取真正api的地址
    ...
    Loop_FindTransfer1:        
      GCI j, TYPE
      CMP $RESULT,50                              ;找到call,ret,jcc,jmp，则执行到此处，再单步过去
      JE FindTransfer1
      CMP $RESULT,60    
      JE FindTransfer1
      CMP $RESULT,70    
    ...
    FindTransfer1:
    CMP j, eip                                    ;如果转移过去之后第一条还是转移指令，就继续单步
